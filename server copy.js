...
app.get('/api/pagesamount/:dbname/:query?', async (req, res, next) => {
	try {
		...
		if (query !== undefined) {
			const decodedQueryParams = decodeQueryParams(query)
			//NAME
			if (decodedQueryParams['name']) {
				(decodedQueryParams['name']) = new RegExp((decodedQueryParams['name']), 'i')
			}	//name is matched with RegExp
	...
	try {
		...
		if (req.params.id) {
			const id = req.params.id,
				...
				result = await db.collection(dbName).findOne({ _id: mongodb.ObjectId(id) })
			res.send(result)
	...
	try {
		...
		if (req.params.dbName !== "messages" && req.params.dbName !== "users" && req.params.dbName !== "sessions" && req.params.dbName !== "confirmations") {
			if (req.params.id) {
				const id = req.params.id,
					dbName = "filters"
				const result = await db.collection(dbName).findOne({ _id: mongodb.ObjectId(id) })	// id było równe mongodb.ObjectId(id)
				res.send(result)
				dbLog('Got results from DB!', 'findOne()', dbName, JSON.stringify({ _id: mongodb.ObjectId(id) }), result)
			}
		}
...
app.post('/api/comment/:dbName', isLoggedIn, async (req, res, next) => {
	try {
		...
		const newID = new mongodb.ObjectID,
			insert = new Comment({ "_id": newID, ...body })
		await insert.save()
		//increase comments amount to + 1
		const _id = req.user._id,
			currentCommentsAmount = await User.findById(_id, 'commentsamount').exec()
...
app.post('/api/newUser/users', async (req, res, next) => {
	try {
		...
		const salt = await bcrypt.genSalt(10),
			...
			buffer = await require('crypto').randomBytes(48)
...
app.post('/api/edituser/', isLoggedIn, async (req, res, next) => {
	try {
		const body = req.body,
			...
			docs = await User.findByIdAndUpdate(user_id, { about: body.about, from: body.from })
...
app.delete('/api/deleteComment/:dbName/:id', isLoggedIn, async (req, res, next) => {
	try {
		const alertText = 'You are not allowed to complete this action.',
			dbName = req.params.dbName,
			id = req.params.id,
			selector = { _id: mongodb.ObjectId(id) },
...
app.get('/api/user-avatar/:name', async (req, res, next) => {
	try {
		...
		let path = await User.findOne({ username: name }, 'avatar')
		dbLog('Getting user Avatar path')
		if (path) {
			path = path.avatar
			path && path.length > 0 ? res.send(path) : res.send(defaultPath)
		}
		else {
